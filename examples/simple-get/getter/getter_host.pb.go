...
func NewGetterPlugin(ctx context.Context, opts ...wazeroConfigOption) (*GetterPlugin, error) {
	o := &WazeroConfig{
...
func (p *GetterPlugin) Load(ctx context.Context, pluginPath string, hostFunctions HostFunctions) (getter, error) {
	...
	apiVersion := module.ExportedFunction("getter_api_version")
	if apiVersion == nil {
		return nil, errors.New("getter_api_version is not exported")
	}
	results, err := apiVersion.Call(ctx)
	if err != nil {
		return nil, err
	} else if len(results) != 1 {
		return nil, errors.New("invalid getter_api_version signature")
	}
	if results[0] != GetterPluginAPIVersion {
		return nil, fmt.Errorf("API version mismatch, host: %d, plugin: %d", GetterPluginAPIVersion, results[0])
	}

	getconfigmap := module.ExportedFunction("getter_get_config_map")
	if getconfigmap == nil {
		return nil, errors.New("getter_get_config_map is not exported")
	}
...
func (p *getterPlugin) Close(ctx context.Context) (err error) {
...
type getterPlugin struct {
	...
	malloc       api.Function
	free         api.Function
	getconfigmap api.Function
}

func (p *getterPlugin) GetConfigMap(ctx context.Context, request *GetRequest) (*GetReply, error) {
	...
	ptrSize, err := p.getconfigmap.Call(ctx, dataPtr, dataSize)
	if err != nil {
		return nil, err
	}
